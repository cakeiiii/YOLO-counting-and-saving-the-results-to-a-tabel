...
class SpeedEstimator:
    ...
    def get_frame_speed_data(self):
        """获取当前帧的速度数据"""
        frame_speed_data = {}
        for track_id, speed in self.dist_data.items():
            if track_id in self.trk_ids:
                class_name = self.names.get(self.clss[self.trk_ids.index(track_id)], 'Unknown')
                if class_name not in frame_speed_data:
                    frame_speed_data[class_name] = []
                frame_speed_data[class_name].append(speed)

        # 计算每个类别的平均速度
        avg_speed_data = {f"{class_name}_speed": sum(speeds) / len(speeds) for class_name, speeds in
                          frame_speed_data.items()}
